...
grafica_avance_entidades <- function(
  ...
  df_plot <- df %>%
    dplyr::mutate(
      ...
      diferencia = pmax(pct_modelo - pct_avance, 0),
  ...
  ggplot2::ggplot(df_plot, ggplot2::aes(y = entidad)) +
    ...
    ggplot2::geom_col(
      ggplot2::aes(x = pct_modelo, fill = color),
      width = 0.78
    ) +
    
    ggplot2::geom_rect(
      data = df_plot %>% dplyr::filter(diferencia > 0),
      ggplot2::aes(
        xmin = pct_avance,
        xmax = pct_modelo,
        ymin = y_num - 0.39,
